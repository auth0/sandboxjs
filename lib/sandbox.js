...
Sandbox.prototype.createLogStream = function (options) {
    ...
    var reconnectableEventSource = Reconnect(function (url, options) {
        console.log('createConnection', url, options);
        var es = EventSource(url, options);
    ...
    var reconnector = reconnectableEventSource();
    
    reconnector.on('connect', function (conn) {
        console.log('connect', conn);
    });
    
    reconnector.on('disconnect', function (err) {
        console.log('disconnect', err);
        
        logs.removeSource();
    });
    
    reconnector.on('reconnect', function (n, delay) {
    ...
    reconnector.connect(url, { json: true });
...
Sandbox.prototype.getCronJob = function (options, cb) {
    options = defaults(options, {
        ...
        handleMetadata: this._createCronJob.bind(this),
    });
