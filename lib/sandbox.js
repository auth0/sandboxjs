...
/**
 * recursive promise for calling back in to get the next page of results
 *
 * @param {Object} options - Options
 * @param {String} [options.container] - Set the webtask container. Defaults to the profile's container.
 * @return {Promise} A Promise that will be fulfilled with an array of Webtasks
 */
Sandbox.prototype.getNextWebtaskList = function (options) {
    var self = this;
    return this.listWebtasks(options)
        .then(function(webtasks) {
            if (webtasks.length === options.limit) {
                /* There could be more out there! */
                options.offset += options.limit;
              return self.getNextWebtaskList(options)
                    .then(function(nextWebtasks) {

                      return webtasks.concat(nextWebtasks);
                    })
                  .catch(function(err) {
                    /* TODO: May want to handle different error conditions differently here */
                    return webtasks;
                  });
            }
            return webtasks;
...
/**
 * List all webtasks from the webtask container
 *
 * @param {Object} options - Options
 * @param {String} [options.container] - Set the webtask container. Defaults to the profile's container.
 * @param {Function} [cb] - Optional callback function for node-style callbacks.
 * @return {Promise} A Promise that will be fulfilled with an array of Webtasks
 */
Sandbox.prototype.listAllWebtasks = function (options, cb) {
    if (!options) options = {};

    if (!options.offset) options.offset = 0;
    if (!options.limit) options.limit = 20;

    var promise = this.getNextWebtaskList(options);
