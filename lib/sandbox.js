...
/**
 * Create a NamedWebtask from the given options
 *
...
Sandbox.prototype.create = function (codeOrUrl, options, cb) {
    ...
    var promise = this.createToken(options)
        .then(function (token) {
            return new Webtask(self, token, options.meta);
        });
...
Sandbox.prototype.createRaw = function (claims, cb) {
    ...
    var promise = this.createTokenRaw(claims)
        .then(function (token) {
            return new Webtask(self, token, claims.meta);
        });
...
Sandbox.prototype.createToken = function (options, cb) {
    ...
    var promise = new Bluebird(function (resolve, reject) {
        ...
        if (options.merge || options.mergeBody)
            params.mb = 1;
        if (options.parse || options.parseBody)
            params.pb = 1;
...
Sandbox.prototype.getWebtask = function (options, cb) {
    ...
    if (!options.name) {
        ...
        promise = Request(request)
            .get('body')
            .then(function (data) {
                return new Webtask(self, data.token, data);
            });
    }
...
Sandbox.prototype.updateWebtask = function (options, cb) {
    ...
    function onWebtask(webtask) {
        return webtask.inspect({ decrypt: options.secrets !== false, fetch_code: options.code !== false })
            .then(onInspection);
        ...
        function onInspection(claims) {
            ...
            if (typeof options.mergeBody !== 'undefined' || typeof options.merge !== 'undefined' || claims.pb) {
...
Sandbox.prototype.inspectWebtask = function (options, cb) {
    ...
    var promise = this.getWebtask({ name: options.name })
        .call('inspect', { decrypt: options.decrypt, fetch_code: options.fetch_code });
...
function webtaskFactory(sandbox, token) {
    var claims = Decode(token);
    
    return claims.jtn
        ?   new NamedWebtask(sandbox, token)
        :   new Webtask(sandbox, token);
}
